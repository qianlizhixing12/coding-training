...
void getNums(string &s, vector<vector<int>> &nums, int n);

int getLargestProduct(vector<vector<int>> &nums, int n);
...
int main(int argc, char *argv[]) {
  ...
  getNums(s, nums, n);
  cout << getLargestProduct(nums, n) << endl;
  return 0;
}
...
void getNums(string &s, vector<vector<int>> &nums, int n) {
  ...
  int row = 0;
  int col = 0;
  while (getline(iss, tmp, seq)) {
    nums[row][col++] = atoi(tmp.c_str());
    if (col == n) {
      row++;
      col = 0;
    }
...
int getLargestProduct(vector<vector<int>> &nums, int n) {
  ...
  for (int i = 0; i < n - 3; i++) {
    for (int j = 0; j < n - 3; j++) {
      ...
      // 右方
      int tmp = nums[i][j] * nums[i][j + 1] * nums[i][j + 2] * nums[i][j + 3];
      result = max(result, tmp);
      // 下方
      tmp = nums[i][j] * nums[i + 1][j] * nums[i + 2][j] * nums[i + 3][j];
      result = max(result, tmp);
      // 右下
      tmp = nums[i][j] * nums[i + 1][j + 1] * nums[i + 2][j + 2] *
            nums[i + 3][j + 3];
      result = max(result, tmp);
      // 左下
      if (j - 3 >= 0) {
